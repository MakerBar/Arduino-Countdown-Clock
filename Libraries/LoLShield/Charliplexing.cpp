...
int n =0;
    ...
    pinDataLo = displayBuffer[i*2];
    pinDataHi = displayBuffer[i*2+1];  

    if (i < 6) {
        pinDirLo = _BV(i+2) | displayBuffer[i*2];
        pinDirHi =            displayBuffer[i*2+1];
    } else {
        pinDirLo =            displayBuffer[i*2];
        pinDirHi = _BV(i-6) | displayBuffer[i*2+1];     
    } 

    //port E mappings 
    DDRE = (DDRE & (~0x38)) | ((pinDirLo << 2) & 0x30) | ((pinDirLo >> 2) & 0x8);
    PORTE = (PORTE & (~0x38)) | ((pinDataLo << 2) & 0x30) | ((pinDataLo >> 2) & 0x8);	
    //port G mappings 
    DDRG = (DDRG & (~0x20)) | ((pinDirLo << 1) & 0x20);
    PORTG = (PORTG & (~0x20)) | ((pinDataLo << 1) & 0x20);
    //port H mappings
    DDRH = (DDRH & (~0x78)) | ((pinDirLo >> 3) & 0x18) | ((pinDirHi << 5) & 0x60);
    PORTH = (PORTH & (~0x78)) | ((pinDataLo >> 3) & 0x18) | ((pinDataHi << 5) & 0x60);
    //port B mappings
    DDRB = (DDRB & (~0xf0)) | ((pinDirHi << 2) & 0xf0);
    PORTB = (PORTB & (~0xf0)) | ((pinDataHi << 2) & 0xf0);	
#else
    // For most Arduino board types, the displayBuffer bits map directly to ports B and D on
    // the microcontroller.
    if (i < 6) {
